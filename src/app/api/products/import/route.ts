...
const CHUNK_SIZE = 1000; // Разбиваем на чанки по 1000 строк
...
export async function POST(req: Request) {
	try {
		...
		for (let i = 1; i < totalRows; i += CHUNK_SIZE) {
			...
			console.log(`Обрабатываем чанк с ${i + 1} по ${i + chunk.length}`);
			...
			for (const [i, row] of chunk.entries()) {
				try {
					...
					if (!sku || !title || !priceRaw || !brand) {
						console.warn(`Пропущен товар: ${row}`);
						continue;
					}
					...
					console.warn(`⚠️ Ошибка при импорте строки ${i + 1}:`, err);
					continue;
				}
			}
		}
