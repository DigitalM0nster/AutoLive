			...
			<table className="w-full table-auto text-sm border border-gray-300">
				<thead className="bg-gray-100">
					<tr>
						...
						<th className="border px-2 py-1">Админ</th>
					...
					{logs.length > 0 ? (
						logs.map((log) => (
							<tr key={log.id}>
								...
								<td className="border px-2 py-1">{log.fileName}</td>
								<td className="border px-2 py-1 text-center">{log.created}</td>
								<td className="border px-2 py-1 text-center">{log.updated}</td>
							</tr>
						))
					) : (
						<tr>
