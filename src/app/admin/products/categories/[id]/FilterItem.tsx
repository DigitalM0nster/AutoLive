// src\app\admin\categories\[id]\edit\FilterItem.tsx
...
export default function FilterItem({ filter, onChange, onDelete }: { filter: LocalFilter; onChange: (filter: LocalFilter) => void; onDelete: () => void }) {
	...
	const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
		onChange({ ...filter, type: e.target.value as any, changed: true });
	};

	const handleValueChange = (index: number, value: string) => {
		const updatedValues = [...filter.values];
		updatedValues[index] = { ...updatedValues[index], value, changed: true, error: "" };
				...
				<div className="px-4 pb-4 space-y-4">
					<div className="space-y-1">
						...
						<input
							...
							className="border rounded px-3 py-2 w-full shadow-sm focus:ring-2 focus:ring-blue-400"
						/>
					</div>

					<div className="space-y-1">
						<label className="block text-sm font-medium text-gray-700">Тип фильтра</label>
						<select value={filter.type} onChange={handleTypeChange} className="border rounded px-3 py-2 w-full shadow-sm">
