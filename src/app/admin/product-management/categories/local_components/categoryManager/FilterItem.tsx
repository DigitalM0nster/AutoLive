// src\app\admin\product-management\categories\local_components\categoryManager\FilterItem.tsx
...
export default function FilterItem({ filter, onChange, onDelete }: FilterItemProps) {
	...
	return (
		<div className="border border-black/10 rounded-xl shadow-sm bg-white overflow-hidden">
			{/* Header */}
				...
				<div className="px-4 pb-4 space-y-4">
					...
					<div className="space-y-1">
						...
						<input
							...
							className={`border border-black/10 rounded px-3 py-2 w-full shadow-sm focus:ring-2 focus:ring-blue-400 ${
								filter.error ? "border-red-500" : "border-gray-300"
							}`}
						/>
					...
					<div className="space-y-1">
						...
						<select
							...
							className={`border border-black/10 rounded px-3 py-2 w-full shadow-sm ${filter.error ? "border-red-500" : "border-gray-300"}`}
						>
										...
										!v.markedForDelete && (
											<li key={vi} className="flex items-center gap-2">
												<input
													...
													className={`border border-black/10 rounded px-3 py-1 w-full text-sm ${v.error ? "border-red-500" : "border-gray-300"}`}
												/>
