		...
		<div className="mb-4">
			...
			<table className="table-auto border border-black/10 text-sm w-full">
				<thead>
					<tr>
						{preview[0].map((_, idx) => (
							<th key={idx} className="border border-black/10 px-2 py-1 bg-gray-100 text-gray-600 font-semibold">
								#{idx + 1}
							</th>
						))}
					</tr>
					...
					{preview.slice(0, 10).map((row, rowIndex) => (
						<tr key={rowIndex}>
							{row.map((cell, cellIndex) => (
								<td key={cellIndex} className="border border-black/10 px-2 py-1">
									{cell}
								</td>
							))}
						</tr>
