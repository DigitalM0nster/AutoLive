...
export default function DepartmentLogsTable({
	...
	const getResultBlock = useCallback(
								...
								<div className={`openingBlock ${activeBlocks[deleteLogKey] ? "active" : ""}`}>
									{log.snapshotBefore && (
										<div className="tableListItems">
											...
											{/* Показываем пользователей, которые остались без отдела */}
											{log.snapshotAfter?.removedUsers && log.snapshotAfter.removedUsers.length > 0 && (
												<div className="tableListItem fullInfoBlock">
													<div
														className={`clickInfoBlock ${activeBlocks[`${deleteLogKey}_users`] ? "active" : ""}`}
														onClick={() => toggleActiveBlock(`${deleteLogKey}_users`)}
													>
														Сотрудники отдела (остались без отдела)
													</div>
													<div className={`openingBlock ${activeBlocks[`${deleteLogKey}_users`] ? "active" : ""}`}>
														<div className="tableListItems">
								...
								<div className={`openingBlock ${activeBlocks[createLogKey] ? "active" : ""}`}>
									<div className="tableListItems">
										...
										{actions.includes("add_employees") && (
											<div className="tableListItem fullInfoBlock">
												<div
													className={`clickInfoBlock ${activeBlocks[`${createLogKey}_users`] ? "active" : ""} addedUser`}
													onClick={() => toggleActiveBlock(`${createLogKey}_users`)}
												>
													Добавленные сотрудники
												</div>
										...
										{actions.includes("remove_employees") && (
											<div className="tableListItem fullInfoBlock">
												<div
													className={`clickInfoBlock removedUser ${activeBlocks[`${createLogKey}_users`] ? "active" : ""}`}
													onClick={() => toggleActiveBlock(`${createLogKey}_users`)}
												>
		...
		[activeBlocks, toggleActiveBlock, getRoleName, getStatusName, existingCategories, renderCategory]
	);
	...
	useEffect(() => {
		const fetchLogs = async () => {
			...
			setError(null);
			try {
				const response = await fetch(`/api/departments/${departmentId}/logs?${queryParams.toString()}`);
		...
		fetchLogs();
	}, [queryParams, checkUsersExistence, checkDepartmentsExistence, checkCategoriesExistence, onLogsUpdate, departmentId]);
