...
export default function AllPickupPointsLogsTable({ tableHeaders, queryParams, onLogsUpdate, adminSearch }: Props) {
									...
									{snap && (
										<>
											<div className="infoField">
												...
												<span className="value">{snap.id}</span>
											</div>
											{snap.name != null && (
												<div className="infoField">
	...
	useEffect(() => {
		const fetchLogs = async () => {
			...
			setError(null);
			try {
				const res = await fetch(`/api/pickup-points/logs?${queryParamsString}`, { credentials: "include" });
				...
				console.error("Ошибка загрузки логов пунктов выдачи:", e);
		...
		fetchLogs();
	}, [queryParamsString, onLogsUpdate]);
					...
					{loading ? (
						...
						localLogs.map((log) => (
							<tr key={log.id}>
								...
								<td>
									<a href={`/admin/pickup-points/${log.pickupPointId}/edit`} className="departmentLink" onClick={(e) => { e.preventDefault(); router.push(`/admin/pickup-points/${log.pickupPointId}/edit`); }}>
										Пункт #{log.pickupPointId}
									</a>
								</td>
